...
(()=>{
 ...
 function install(){
  ...
  const raw=fn.bind(api);function deterministicSetPart(type,values={}){if(!ALLOWED.has(type))return raw(type,values);const before=captureValues(type);resetValues(type);const finish=value=>{if(value===false||failed()){restoreValues(before);if(value===false)return false;throw Error($("status")?.textContent||"CAD-malli ei läpäissyt tarkistusta.")}return value};try{const result=raw(type,values);if(result&&typeof result.then==="function")return Promise.resolve(result).then(finish,error=>{restoreValues(before);throw error});return finish(result)}catch(error){restoreValues(before);throw error}}
