...
(()=>{
 ...
 function blockUnsafeExports(){for(const id of ["btnDownload","btnFitTest","btnCentauriStl"]){const e=$(id);if(e)e.disabled=true}}
 function stale(){blockUnsafeExports();const e=$("planSyncStatus");if(e){e.textContent="⚠ "+STALE_CAD_WARNING;e.className="plan-sync-status warn"}return false}
 function install(){
  const api=window.AI3D,fn=api?.setPart;if(typeof fn!=="function")return false;if(fn.__ai3dProjectDefaults===true)return true;
  const raw=fn.bind(api);function deterministicSetPart(type,values={}){if(!ALLOWED.has(type))return raw(type,values);const token=++setPartSeq,before=captureValues(type);resetValues(type);const finish=value=>{if(token!==setPartSeq)return stale();if(value===false||failed()){restoreValues(before);blockUnsafeExports();if(value===false)return false;throw Error($("status")?.textContent||"CAD-malli ei läpäissyt tarkistusta.")}return value};try{const result=raw(type,values);if(result&&typeof result.then==="function")return Promise.resolve(result).then(finish,error=>{if(token!==setPartSeq)return stale();restoreValues(before);blockUnsafeExports();throw error});return finish(result)}catch(error){if(token!==setPartSeq)return stale();restoreValues(before);blockUnsafeExports();throw error}}
 ...
 function apply(p){if(!validProject(p))throw Error("Projektidata ei läpäissyt rakennetarkistusta.");if(!install())throw Error("CAD-moottori ei ole vielä valmis.");const before=captureValues(p.type),rollback=()=>restoreValues(before),finish=()=>{if(failed())throw Error($("status")?.textContent||"CAD-malli ei läpäissyt tarkistusta.");return true};try{const result=window.AI3D.setPart(p.type,p.values||{});if(result&&typeof result.then==="function")return Promise.resolve(result).then(value=>{if(value===false)throw Error("CAD-moottori hylkäsi projektin.");return finish()}).catch(error=>{rollback();blockUnsafeExports();throw error});if(result===false)throw Error("CAD-moottori hylkäsi projektin.");return finish()}catch(error){rollback();blockUnsafeExports();throw error}}
