...
(()=>{
 ...
 function install(){
  ...
  const raw=fn.bind(api);function deterministicSetPart(type,values={}){if(!ALLOWED.has(type))return raw(type,values);const before=captureValues(type);resetValues(type);try{const result=raw(type,values);if(result&&typeof result.then==="function")return Promise.resolve(result).then(value=>{if(value===false)restoreValues(before);return value},error=>{restoreValues(before);throw error});if(result===false)restoreValues(before);return result}catch(error){restoreValues(before);throw error}}
