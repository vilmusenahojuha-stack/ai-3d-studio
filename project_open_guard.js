...
(()=>{
 ...
 function install(){
  ...
  const raw=fn.bind(api);function deterministicSetPart(type,values={}){if(!ALLOWED.has(type))return raw(type,values);const token=++setPartSeq,before=captureValues(type);resetValues(type);const finish=value=>{if(token!==setPartSeq){blockUnsafeExports();return false}if(value===false||failed()){restoreValues(before);blockUnsafeExports();if(value===false)return false;throw Error($("status")?.textContent||"CAD-malli ei läpäissyt tarkistusta.")}return value};try{const result=raw(type,values);if(result&&typeof result.then==="function")return Promise.resolve(result).then(finish,error=>{if(token!==setPartSeq){blockUnsafeExports();return false}restoreValues(before);blockUnsafeExports();throw error});return finish(result)}catch(error){if(token!==setPartSeq){blockUnsafeExports();return false}restoreValues(before);blockUnsafeExports();throw error}}
