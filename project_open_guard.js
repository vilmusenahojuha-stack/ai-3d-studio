...
(()=>{
 ...
 function apply(p){if(!validProject(p))throw Error("Projektidata ei läpäissyt rakennetarkistusta.");if(!install())throw Error("CAD-moottori ei ole vielä valmis.");const finish=()=>{if(failed())throw Error($("status")?.textContent||"CAD-malli ei läpäissyt tarkistusta.");return true},result=window.AI3D.setPart(p.type,p.values||{});if(result&&typeof result.then==="function")return Promise.resolve(result).then(value=>{if(value===false)throw Error("CAD-moottori hylkäsi projektin.");return finish()});if(result===false)throw Error("CAD-moottori hylkäsi projektin.");return finish()}
