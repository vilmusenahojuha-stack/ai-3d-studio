...
(()=>{
 ...
 function warn(text){lastIssue=text;const box=$("planSyncStatus");if(box){box.textContent="⚠ "+text;box.className="plan-sync-status warn"}}
 ...
 function verify(requireLive=false){
  ...
  try{
   ...
   const items=read(),storedProject=items?.find(p=>p&&p.id===active.id),activeId=localStorage.getItem(KEY+":active")||"";
