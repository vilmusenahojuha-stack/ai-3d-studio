"use strict";
(()=>{
 const KEY="ai3d:projects:v3",$=id=>document.getElementById(id),ALLOWED=new Set(["lightSign","spike","plug","sleeve","plate","adapter","enclosure"]),BLOCKED_KEYS=new Set(["__proto__","prototype","constructor"]),MAX_PROJECT_NAME=120,MAX_PROJECT_DESCRIPTION=1000,MAX_DATE_MS=8.64e15,PRINT_KEYS=new Set(["printer","nozzle","layer","walls","infill","notes"]);
 ...
 const INVALID_LIVE_WARNING="Muokatussa numeerisessa CAD-kentässä on tyhjä, virheellinen tai sallittujen rajojen ulkopuolinen arvo. Korjaa kenttä ennen projektin vaihtamista tai tallennusta.";
 ...
 function hasInvalidLiveNumber(){const active=window.AI3DProjects?.active?.();if(!active?.id)return false;const type=$("partType")?.value||active.type;for(const id of MAP[type]||[]){const e=$(id);if(e?.type!=="number")continue;const n=e.valueAsNumber;if(!Number.isFinite(n))return true;const min=e.min!==""?Number(e.min):NaN,max=e.max!==""?Number(e.max):NaN;if(Number.isFinite(min)&&n<min)return true;if(Number.isFinite(max)&&n>max)return true}return false}
 ...
 window.AI3DStorageCommitGuard={verify,get lastIssue(){return lastIssue},get hasConflict(){const issue=blockingIssue();if(!issue)return false;if(issue!==lastIssue)warn(issue);return true}};
