...
assert.match(index,/plan_operation_guard\.js\?v=1\.10/,"index.html must load the current ChatGPT operation guard cache version");
...
assert.match(runtime,/"plan_operation_guard\.js\?v=1\.10"/,"runtime fallback must use the same ChatGPT operation guard version");
...
assert.match(operations.src,/^plan_operation_guard\.js\?v=1\.10&healthRetry=\d+$/,"ChatGPT operation guard recovery must be cache-busted without changing its pinned version");
...
console.log("CAD v2, ChatGPT guards and Centauri runtime recovery regression: ok");
