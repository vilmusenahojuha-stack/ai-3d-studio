...
assert.equal(validate(plan({ holes: [{ x: "0", y: 0, diameter: 10 }] })).ok, false,
  "numeric-string listed hole x must be rejected before CAD");
assert.equal(validate(plan({ holes: [{ x: 0, y: "0", diameter: 10 }] })).ok, false,
  "numeric-string listed hole y must be rejected before CAD");
assert.equal(validate(plan({ holes: [{ x: 0, y: 0, diameter: "10" }] })).ok, false,
  "numeric-string listed hole diameter must be rejected before CAD");
...
assert.equal(validate(plan({ centerHole: { diameter: "10" } })).ok, false,
  "numeric-string centerHole diameter must be rejected before CAD");
...
assert.equal(validate(plan({}, [{ type: "hole", x: "0", y: 0, diameter: 10 }])).ok, false,
  "numeric-string hole-operation coordinates must be rejected");
...
assert.equal(validate(plan({}, [{ type: "holes", holes: [{ x: 0, y: 0, diameter: "10" }] }])).ok, false,
  "numeric-string holes-operation diameter must be rejected");
