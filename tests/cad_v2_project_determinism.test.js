...
// A previous project must not leak stale dimensions or material into a partial saved/imported v2 project.
elements.adapterID2.value="777";
elements.adapterOD2.value="888";
elements.material.value="PLA";
context.window.AI3D.setPart("adapter",{adapterLength:45,adapterID1:22,adapterOD1:30,material:"ASA"});
...
assert.equal(elements.material.value,"ASA");
assert.equal(lastPlan.material,"ASA");
...
// The same deterministic reset is required for enclosure projects; missing material returns to the startup default.
...
assert.equal(elements.material.value,"PETG");
assert.equal(lastPlan.material,"PETG");
assert.deepEqual(JSON.parse(JSON.stringify(lastPlan.parameters)),{width:100,length:60,height:40,wall:3,floorThickness:2.4});

// Unsupported project material must be rejected before changing the active v2 editor state.
const beforeType=elements.partType.value,beforeMaterial=elements.material.value,beforePlan=lastPlan;
assert.throws(()=>context.window.AI3D.setPart("adapter",{adapterLength:35,material:"ABS"}),/tulostusmateriaalia ei tueta/);
assert.equal(elements.partType.value,beforeType);
assert.equal(elements.material.value,beforeMaterial);
assert.strictEqual(lastPlan,beforePlan);
