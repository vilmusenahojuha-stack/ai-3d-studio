...
(async()=>{
  ...
  await app.context.window.AI3DProjects.openPlan("chatgpt-current");
  ...
  const aliasPlan={schemaVersion:2,projectName:"Alias-päätytulppa",summary:"Schema v2 alias pitää voida muokata projektina",partType:"plug",material:"PETG",parameters:{width:40,height:30,wall:2,clearance:.25,insertDepth:12,capThickness:3,overhang:1}};const aliasStorage=new MemoryStorage({[KEY]:JSON.stringify([originalProject]),[ACTIVE]:originalProject.id});const aliasApp=await boot(aliasStorage,aliasPlan);await aliasApp.context.window.AI3DProjects.openPlan("chatgpt-current");const aliasProjects=JSON.parse(aliasStorage.getItem(KEY));assert.strictEqual(aliasProjects.length,2);const aliasCopy=aliasProjects.find(p=>p.id!==originalProject.id);assert.ok(aliasCopy);assert.strictEqual(aliasCopy.type,"plug");assert.strictEqual(aliasCopy.values.tubeW,40);assert.strictEqual(aliasCopy.values.tubeH,30);assert.strictEqual(aliasCopy.values.tubeWall,2);assert.strictEqual(aliasCopy.values.insertDepth,12);assert.ok(aliasApp.setPartCalls.some(x=>x.type==="plug"));
