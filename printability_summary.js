...
(()=>{
 ...
 function render(){
  ...
  const type=$("partType")?.value||"",build=Array.isArray(window.CentauriProfile?.build)?window.CentauriProfile.build:BUILD,fits=b.dims.every((x,i)=>x<=build[i]+1e-6),pa=printability(m,b),thin=thinnestKnown(),comp=type==="lightSign"?null:componentInfo(m),ori=orientation(),mat=$("material")?.value||"PETG",items=[],warnings=[],incomplete=[];
  ...
  if(thin){if(thin[1]<NOZZLE*2){items.push(`⚠ ${thin[0]} on hyvin ohut (${thin[1].toFixed(2)} mm)`);warnings.push("thin")}else items.push(`✓ Tunnettu minimipaksuus on järkevä 0,4 mm suuttimelle (${thin[0]} ${thin[1].toFixed(2)} mm)`)}else{items.push("ℹ Kaikkien paikallisten seinämäpaksuuksien automaattinen mittaus ei ole vielä mahdollinen.");incomplete.push("thickness")}
  if(pa){if(pa.overhangPct>=18){items.push(`⚠ Vaikeita alaspäin suuntautuvia pintoja on arviolta ${pa.overhangPct.toFixed(1)} % – tukia tai toinen asento voi olla tarpeen`);warnings.push("overhang")}else if(pa.overhangPct>=6){items.push(`⚠ Ylityksiä on jonkin verran (${pa.overhangPct.toFixed(1)} %) – tarkista slicerissa`);warnings.push("overhang")}else items.push(`✓ Ei havaittu paljon vaikeita ylityksiä (${pa.overhangPct.toFixed(1)} %)`);if(pa.bedArea<1){items.push("⚠ Selvää vaakasuoraa pohjakosketusta ei tunnistettu");warnings.push("bed")}else items.push(`✓ Alustakosketusta tunnistettiin noin ${pa.bedArea.toFixed(1)} mm²`)}else{items.push("ℹ Ylitysarviota ei saatu muodostettua.");incomplete.push("overhang")}
  if(comp){if(comp.count>1){items.push(`⚠ Meshissä havaittiin ${comp.count} erillistä geometriaryhmää${comp.small?`, joista ${comp.small} on hyvin pieni`:""}`);warnings.push("components")}else items.push("✓ Mesh muodostaa yhden yhtenäisen geometriaryhmän")}else if(type!=="lightSign"){items.push("ℹ Erillisiä geometriaryhmiä ei voitu tässä mallissa luotettavasti analysoida.");incomplete.push("components")}
  if(ori)items.push(`${ori.kind==="ok"?"✓":ori.kind==="warn"?"⚠":"ℹ"} Suositeltu tulostusasento: ${ori.text}`);else{items.push("ℹ Tulostusasennon suositusta ei saatu muodostettua.");incomplete.push("orientation")}
  items.push(`ℹ ${materialNotes[mat]||materialNotes.PETG}`);
  if(incomplete.length)items.push("ℹ Arvio jäi osittaiseksi. Tarkista nämä kohdat erityisen huolellisesti slicerin esikatselussa.");
  const good=fits&&warnings.length===0&&incomplete.length===0;root.className="printer-status "+(good?"ok":"");root.innerHTML=`<b>${good?"TULOSTETTAVUUS: HYVÄ ✓":"TULOSTETTAVUUS: HUOMIO"}</b>${items.map(x=>`<span>${x}</span>`).join("")}<span>Studio ei tee slicerin lopullista tukianalyysiä eikä muuta STL:n orientaatiota automaattisesti.</span>`
 }
