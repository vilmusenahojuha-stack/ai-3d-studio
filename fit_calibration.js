...
(()=>{
 ...
 function render(preserveDraft=false){
  ...
  if(storageError){if(!preserveDraft)input.value="";root.className="printer-status fail";root.innerHTML=`<b>Kalibrointidataa ei käytetty.</b><span>${storageError}</span><span>Alkuperäistä paikallista tietuetta ei ylikirjoiteta automaattisesti. Korjaa tai poista vioittunut tietue ennen uuden kalibroinnin tallennusta.</span>`;return}
  const mat=material(),saved=correction(),c=clearance(),s=suggestion();if(!preserveDraft)input.value=Number.isFinite(saved)?String(saved):"";
