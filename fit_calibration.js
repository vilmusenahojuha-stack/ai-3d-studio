...
(()=>{
 ...
 function render(){
  ...
  root.className="printer-status ok";root.innerHTML=`<b>${mat}: käyttäjän kalibrointi tallennettu</b><span>Korjaus ${saved>=0?"+":""}${saved.toFixed(2)} mm / puoli • suutin ${NOZZLE.toFixed(1)} mm</span><span>${Number.isFinite(s)?`Nykyinen välys ${c.toFixed(2)} mm → ehdotus ${s.toFixed(2)} mm / puoli.`:"Muuta piikkimutterin välystä nähdäksesi ehdotuksen."}</span><span>Ehdotus on muistutus käyttäjän omasta testituloksesta. Arvoa ei sovelleta automaattisesti.</span>`
 }
 function saveCurrent(){
  ...
  const mat=material(),prev=state.corrections[mat];state.corrections[mat]={correction:x,updatedAt:Date.now()};try{write();render()}catch(e){if(prev)state.corrections[mat]=prev;else delete state.corrections[mat];root.className="printer-status fail";root.innerHTML=`<b>Kalibrointia ei tallennettu.</b><span>${e?.message||"Paikallinen tallennus epäonnistui."}</span>`}
 }
 function clearCurrent(){const mat=material(),root=$("fitCalibrationState"),prev=state.corrections[mat];delete state.corrections[mat];try{write();render()}catch(e){if(prev)state.corrections[mat]=prev;if(root){root.className="printer-status fail";root.innerHTML=`<b>Kalibrointia ei poistettu.</b><span>${e?.message||"Paikallinen tallennus epäonnistui."}</span>`}}}
 function init(){read();inject();render();$("partType")?.addEventListener("change",render);$("material")?.addEventListener("change",render);$("clearance")?.addEventListener("input",()=>{const input=$("fitCorrection"),active=document.activeElement===input;if(!active)render()});document.addEventListener("visibilitychange",()=>{if(!document.hidden){read();render()}})}
 window.AI3DFitCalibration={render,getCorrection:mat=>MATERIALS.has(mat)?state.corrections[mat]?.correction:null,getSuggestion:()=>suggestion()};
