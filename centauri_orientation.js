"use strict";
(()=>{
 if(!document.querySelector('script[data-ai3d-plan-preflight]')){const s=document.createElement("script");s.src="plan_preflight.js?v=1.0";s.dataset.ai3dPlanPreflight="1";document.head.appendChild(s)}
 ...
 function analyse(){const m=mesh(),api=window.CentauriProfile;if(!m||!api?.bounds||!api?.printability)return[];const build=Array.isArray(api.build)&&api.build.length===3?api.build:[256,256,256];return views.map(v=>{const tm=transformed(m,v.rot),b=api.bounds(tm),dims=b?.dims||[0,0,0],fits=dims.every((x,i)=>Number.isFinite(x)&&x>0&&x<=build[i]+1e-6),pa=b?api.printability(tm,b):null,r={...v,dims,fits,printability:pa,build};r.score=scoreResult(r);return r}).sort((a,b)=>a.score-b.score)}
