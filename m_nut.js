...
(()=>{
 ...
 function buildM(values){
  ...
  // Piikki kapenee normaalisti lähes kärkeen asti.
  const N=64,morphStart=p.totalHeight-p.monogramHeight;
  const cone0=circleRing(outerAF/2*1.02,p.baseHeight-1.2,N);
  const cone1=circleRing(Math.max(4.6,p.monogramWidth*.28),morphStart,N);
  bridge(T,cone0,cone1);

  // Kärjen poikkileikkaus muuttuu ympyrästä M-kirjaimeksi. M näkyy siis YLÄ-näkymässä.
  const raw=[[-.50,-.50],[-.28,-.50],[-.28,.15],[0,-.18],[.28,.15],[.28,-.50],[.50,-.50],[.50,.50],[.28,.50],[0,.12],[-.28,.50],[-.50,.50]];
  const shaped=raw.map(([x,y])=>[x*p.monogramWidth,y*p.monogramDepth]);
  const mr=resampleClosed(shaped,N),mRing=mr.map(q=>v(q[0],q[1],p.totalHeight));
  bridge(T,cone1,mRing);
  capPolygon(T,mr,p.totalHeight,true);
 ...
 function finish(mesh){currentFitMesh=null;currentMesh=mesh;const a=validate(mesh),ok=a.ok;el("validation").innerHTML=`<div class="check ${ok?"ok":"fail"}"><strong>${ok?"✓ Automaattitarkistus OK":"✕ Tarkistus epäonnistui"}</strong><br>${a.message}<br>Kartion kärjen poikkileikkaus muuttuu M-kirjaimeksi. Tarkista erityisesti YLÄ-näkymä.</div>`;el("btnDownload").disabled=!ok;el("btnFitTest").disabled=true;el("status").textContent=ok?"60 mm M-piikkimutterisuojus luotu. Paina YLÄ nähdäksesi kärjen M-muodon.":"STL-lataus estetty virheen vuoksi.";el("dimensions").textContent=mesh.measure.map(([k,x])=>`${k} ${typeof x==="number"?x.toFixed(2)+" mm":x}`).join(" • ");updateMaterial();draw()}
